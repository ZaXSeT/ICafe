...
export function AuthTabs() {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (code === "auth/invalid-credential" || code === "auth/user-not-found" || code === "auth/wrong-password") {
        ...
        toast.error("Email not verified", { description: "Please check your inbox (or Spam folder) and verify your email first." });
      } else {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Account created!", { description: "Please check your email (and Spam folder) to verify your account." });
      setTab("signin");
      setEmail(regEmail);
    } catch (error: any) {
      ...
      if (code === "auth/email-already-in-use") {
        toast.error("Email already in use", { description: "Please sign in or use a different email." });
      ...
      {tab === "signin" && (
        <div>
          ...
          <form onSubmit={handleSignIn} className="space-y-4">
            ...
            <button type="button" onClick={handleGoogleSignIn} className="w-full h-11 rounded-xl border border-border/40 bg-background text-foreground font-semibold text-sm flex items-center justify-center gap-2 hover:bg-foreground/5 transition-all">
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input id="name" type="text" placeholder="John Doe" value={regName} onChange={e => setRegName(e.target.value)} required className={inputClass} />
              </div>
            </div>
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <Mail className={iconClass} />
                <input id="reg-email" type="email" placeholder="you@example.com" value={regEmail} onChange={e => setRegEmail(e.target.value)} required className={inputClass} />
              </div>
            </div>
            <div className="space-y-1.5">
